...
Ex : $R=\{(a,b),\text{student  a is taking class b}\}$

Symbol:
$(a,b)\in R, aRb$ 

A relation on $A$ is a subset $R\subseteq A\times A$ .
Ex: $A=\mathcal{Z}: xRy \text{ iff } x\equiv y \pmod 5$
...
>
>2.$[a] \neq \phi$
>Since $R$ is reflexive, $[a]$ has at least one element $a$.
>
>3.Disjointness
...
>By lemma, $A$ with $n$ elements has minimal $m$ . Consider $A$ \ $\{m\}$ $=A'$ . Restrict $≼$ on $A'\times A'$ : $≼'=≼ \cap A'\times A'$ . By $P(k)$ , $(A',≼')$ has a topological sort $a_2,...a_{k-1}$ . 
...
例如 : $R=\{(a,b),\text{student  a is taking class b}\}$

符号:
$(a,b)\in R, aRb$ 

一个 $A$ 上的关系则是 $A\times A$ 的子集 $R$。这可以用有向图来表示。 $G=(V,E)$ , with $V=A,E=R$ .
...
>由自反性, 对 $a\in A$ 有 $aRa$ . 所以每个元素都有他们自己的等价类，因此 $A \subseteq \bigcup_{a \in A} [a]$
>同时每个等价类都是 $A$ 的子集，所以 $\bigcup_{a \in A} [a] \subseteq A$。
...
一个关系是偏序关系如果他是自反、反对称、传递的。用 $\preccurlyeq$ 表示。


$(A,\preccurlyeq)$ 叫做 **偏序集**.
