...
- $\lambda^1: B_0 - AB_1 = c_1I$
    
- $\lambda^0: -AB_0 = c_0I$
    
为了构造 $p(A)$，我们将上述等式依次左乘 $A^n, A^{n-1}, \dots, A, I$：
- $A^n(B_{n-1}) = A^n$
    
- $A^{n-1}(B_{n-2} - AB_{n-1}) = c_{n-1}A^{n-1}$
    
- $A^{n-2}(B_{n-3} - AB_{n-2}) = c_{n-2}A^{n-2}$
...
将这 $n+1$ 个等式全部相加。你会惊喜地发现，左边所有的 $B_i$ 项都形成了消去项（例如 $A^{n-1}B_{n-2}$ 与下一项中的 $-A \cdot A^{n-2}B_{n-2}$ 抵消）。
...
这就证明了结论，简短美丽得多。另有一种证明，可见[[数学随笔10#**证明：Cayley-Hamilton 定理（连续性方法）**]]  。 另外，有一种证明直接从结构出发，但是需要更多的前置知识，见[[笔记6.线性变换 II——从不变子空间到 Hamilton Cayley 到主分解定理]]。
