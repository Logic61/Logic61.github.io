...
显然，此时有：
* $\dim(V \cap W) = r$
* $\text{rank}(B) = \dim(V) = r + s$
* 我们目标是证明 $\text{rank}(AB) = s$
...
* 结合 $(\beta_1 \dots \beta_n) = (\alpha_1 \dots \alpha_n) B$
...
我们需要从 $v_2 = x^2$ 中减去它在 $P_0$ 和 $P_1$ 方向上的投影：

$$P_2(x) = x^2 - \frac{\langle x^2, P_0 \rangle}{\langle P_0, P_0 \rangle} P_0 - \frac{\langle x^2, P_1 \rangle}{\langle P_1, P_1 \rangle} P_1$$

1. $\langle x^2, P_0 \rangle = \int_{-1}^{1} x^2 \cdot 1 dx = \left[ \frac{1}{3}x^3 \right]_{-1}^{1} = \frac{2}{3}$
    
2. $\langle P_0, P_0 \rangle = \int_{-1}^{1} 1 \cdot 1 dx = 2$
    
3. $\langle x^2, P_1 \rangle = \int_{-1}^{1} x^2 \cdot x dx = 0$（奇函数积分为 0）
