...
\begin{bmatrix} \lambda_1 & \mathbf{0} \ \mathbf{0} & B \end{bmatrix}
$$
</p>
...
(由于$P^T A P$仍为对称矩阵，故第一行其余元素必为$0$)
  ...
  3. **套用归纳假设** ：对于$n-1$阶实对称矩阵$B$，存在正交阵$P_1$使得$P_1^T B P_1 = D_1$。 
...
\begin{bmatrix} 1 & \mathbf{0} \ \mathbf{0} & P_1 \end{bmatrix}

\begin{bmatrix} \lambda_1 & \mathbf{0} \ \mathbf{0} & D_1 \end{bmatrix}
...
**A. 基础步骤 ($n =1$)**$1 \times 1$矩阵本身就是上三角阵，结论显然成立。
...
\begin{bmatrix} \lambda_1 & \mathbf{w}^H \ \mathbf{0} & A_{n-1} \end{bmatrix}
$$
</p>
