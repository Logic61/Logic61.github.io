...
![[Pasted image 20260426212347.png]]
...
![[Pasted image 20260426212806.png]]

![[Pasted image 20260426212848.png]]
...
![[Pasted image 20260426213944.png]]

同时，利用我们在商空间证明的 $$\text{Im}\mathcal{A} \big/ (\text{Im}\mathcal{A} \cap \text{Ker}\mathcal{B}) \cong \text{Im}(\mathcal{BA})$$
我们代入 $A=B^{k-1}$ 就得到
$$
...
实际上，由幂零变换我们就可以给出空间的分解，正是利用循环子空间![[Pasted image 20260426214159.png]]
我们考虑基底扩张的过程，从最小的 $$W_k = \text{Im}\mathcal{B}^{k-1} \cap \text{Ker}\mathcal{B}$$ 中，我们取一组基 $\eta_{1}^{(t)}, \dots, \eta_{m_t}^{(t)}$ ，然后我们扩充到下一级，在 $W_{t-1}$ 补上 $\eta_{1}^{(t-1)}, \dots$；..直到 $W_1$。 **此时，所有 $\eta$ 组成的集合是 $\text{Ker}\mathcal{B}$ 的基，且彼此线性无关。**
...
![[Pasted image 20260426214212.png]]我们需要确定这些直和在一起是否填满了整个 $V$。
...
- **在 $W_{t-1}$ 但不在 $W_t$ 里的向量：** 它们只能往回追溯 $t-2$ 步。它们生成的是长度为 $t-1$ 的链。

$\dim W_k$ 实际上是“长度 $\ge k$ 的循环链”的数量。这让我们联想到期望公式，对于正整数的随机变量
...
![[Pasted image 20260426215909.png]]

# Jordan 标准型

![[Pasted image 20260426224042.png]]

![[Pasted image 20260426224100.png]]
...
![[Pasted image 20260426224957.png]]

降幂排列的话，$1$ 就在右上角。
![[Pasted image 20260426225028.png]]
...
![[Pasted image 20260426230259.png]]![[Pasted image 20260426230307.png]]
![[Pasted image 20260427154849.png]]
...
![[Pasted image 20260427152316.png]]
...
![[Pasted image 20260427152944.png]]
...
![[Pasted image 20260427153739.png]]

如果大的块是零矩阵，那么小的块自动已经为零，所以考虑 $J$ 的幂次是否为零，我们只需要考虑同一个特征值中最大的块。

![[Pasted image 20260427153847.png]]
