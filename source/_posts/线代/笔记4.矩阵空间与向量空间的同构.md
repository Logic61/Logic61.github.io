...
- $A \otimes B = [a, b, 2a, 2b, 3a, 3b]^T$
    
- $B \otimes A = [a, 2a, 3a, b, 2b, 3b]^T$
...
- **正交性保持**：若 $A, B$ 是正交矩阵，则 $A \otimes B$ 也是正交矩阵。
...
1. **秩**：$\text{rank}(A \otimes B) = \text{rank}(A) \cdot \text{rank}(B)$
    
2. **行列式**：设 $A$ 为 $m$ 阶方阵，$B$ 为 $n$ 阶方阵，则 $|A \otimes B| = |A|^n |B|^m$
...
- $A_4 = A_2 \otimes A_2 = \frac{1}{2} \begin{pmatrix} A_2 & A_2 \\ A_2 & -A_2 \end{pmatrix}$
    
- $A_8 = A_2 \otimes A_4$ ... 以此类推。
...
- **特征值计算**：
    ...
    - $I_n \otimes A$ 的特征值为 $1 \cdot \lambda_i = \lambda_i$
        
    - $B^T \otimes I_m$ 的特征值为 $\mu_j \cdot 1 = \mu_j$
        
    - $(B^T)^2 \otimes A$ 的特征值为 $\mu_j^2 \cdot \lambda_i$
        

由于这些项在同一组基（由 $A$ 和 $B^T$ 的特征向量构成的张量积基）下可以同时三角化，因此变换 $\mathcal{S}$ 的特征值为：
