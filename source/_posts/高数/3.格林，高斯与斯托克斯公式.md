...
取 $\Sigma$ 为球面 $x^2 + y^2 + z^2 = 1$ 在第一卦限的部分。其单位外法向量为 $\mathbf{n} = (x, y, z)$。
...
![[Pasted image 20260402122823.png]]
...
![[Pasted image 20260402122951.png]]
...
> 这种方法的本质是：将“对面积的积分”转化为“沿边界的环流积分”，这在处理复杂边界时往往比直接做二重积分更**优雅**。
...
这里的 $\wedge$ 是所谓**外微分**，这在后续笔记中将简单提到，可以先简单理解成代表了方向的 $dzdx$ 。
...
- $C = \frac{\partial(x, y)}{\partial(u, v)} = x_u y_v - x_v y_u$
    
- $B = \frac{\partial(z, x)}{\partial(u, v)} = z_u x_v - z_v x_u$
