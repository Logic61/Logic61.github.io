...
**例 2**：$\int_0^{+\infty} e^{-x^2} dx$拆分$[0,1]$（连续，定积分存在）和 $[1, +\infty)$。当 $x \geq 1$时$e^{-x^2} \leq e^{-x}$，而 $\int_1^{+\infty} e^{-x} dx$收敛，故原积分**收敛**。（精确值为$\frac{\sqrt{\pi}}{2}$）
...
1. 取上限 $A = N\pi$，缩小区间至 $[\pi, N\pi]$2. 拆分为$\sum_{k=1}^{N-1} \int_{k\pi}^{(k+1)\pi} \frac{|\sin x|}{x^p} dx$

3. 分母放大：$\geq \sum_{k=1}^{N-1} \frac{1}{((k+1)\pi)^p} \int_{k\pi}^{(k+1)\pi} |\sin x| dx$4.$\int_{k\pi}^{(k+1)\pi} |\sin x| dx = 2$（半周期面积恒为 2）
...
- **右端点为瑕点**：$\int_a^b f(x) dx = \lim_{\delta \to 0^+} \int_a^{b-\delta} f(x) dx$- **内部瑕点**$c \in (a, b)$：收敛 $\Longleftrightarrow$ $\int_a^c f$和$\int_c^b f$ **各自独立收敛**：
